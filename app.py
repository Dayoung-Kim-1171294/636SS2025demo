...
def book_search():
    ...
    if where_conditions:
        ...
        try:
            ...
            print("Query executed successfully:", cursor.statement)
        except Exception as e:
            print("Query executed failed:", cursor.statement)
...
def loan():
    """Displays initial loan page.
        Adds loan if borrower, book and book copy have all been selected."""
    cursor = db.get_cursor()
    ...
    # List all books
    # (Ideally this would only list available book copies, but the query
    #  would be too complex for this course.
    #  Error displayed in loan_select_book if no copies available.)
    books_qstr = """
...
def loan_select_book():
    """Called from loan page when book copies are required.
        If no copies are available, book is cleared 
        and a flash message asks user to select another book."""
    cursor = db.get_cursor()
    ...
    # (Ideally this would only list available book copies, but the query
    #  would be too complex for this course.
    #  Error displayed below if no copies available.)
    books_qstr = """
    SELECT bookid, booktitle, author
    FROM books 
    ORDER BY booktitle;
...
def borrower_list():
    """Display search form then list of borrowers when form submitted.
        All borrowers shown if search criteria are empty."""
    cursor = db.get_cursor()
...
def borrower_manage(borrower_id=None):
    """Display borrower details form. Form is empty for new borrowers;
        populated with existing values if a borrower id is present."""
